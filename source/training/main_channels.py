...
from source.code_from_TAs.mask_to_submission import patch_to_label

NUM_CHANNELS = 6 # RGB images
PIXEL_DEPTH = 255
NUM_LABELS = 2
TRAINING_SIZE = 10
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    def get_prediction_for_patch(filename, image_idx):
        ...
        data = numpy.asarray(img_crop(img_new, IMG_PATCH_SIZE, IMG_PATCH_SIZE))
        data_node = tf.constant(data)
        output = tf.nn.softmax(model(data_node))
        output_prediction = tf_session.run(output)
    ...
    def get_prediction_with_groundtruth(filename, image_idx):
        ...
        img_prediction = get_prediction(img_new)
    ...
    with tf.Session() as tf_session:
        ...
        prediction_training_dir = "../../predictions_dir/"
        ...
        # [value_to_class(numpy.mean(data[i])
        y = [label[0] for label in extract_labels(train_labels_filename, TRAINING_SIZE)]
        score = f1_score(y, y_pred, average='weighted')
        print("Final score achieved: ", score)

        #TODO masks_to_submission('submission-test.csv', )
