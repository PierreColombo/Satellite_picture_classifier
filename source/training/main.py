...
from source.preprocessing.preprocessing import generate_features, get_number_of_channels
...
IMG_PATCH_SIZE = 16
CROP_STEP = 4 # Needs to be even divisor of IMG_PATCH_SIZE
...
def extract_data(indices, filename):
    ...
    for i in indices:
        ...
        if os.path.isfile(image_filename):
            ...
            img = generate_features(image_filename)
            imgs.append(img)
        else:
...
def value_to_class(v):
    #TODO: how is this influencing?
    #foreground_threshold = DEFAULT_THRESHOLD #TODO changed from 0.25 # percentage of pixels > 1 required to assign a foreground label to a patch
    df = numpy.sum(v)
...
def full_run(train_indices, val_indices, thresh):
    ...
    if TEST_RUN:
        ...
        # test_data = extract_data(range(1, TEST_SIZE+1), test_dir)
    ...
    def save_imagery(filename, image_idx, prediction_dir):
        ...
        imageid = "satImage_%.3d" % image_idx
        image_filename = filename + imageid + ".png"
        orig_img = mpimg.imread(image_filename)
        features = generate_features(image_filename)
        img = features

        img_prediction = get_prediction(img)

        pimg = get_prediction_with_groundtruth(orig_img, img_prediction)
    ...
    with tf.Session() as tf_session:
            ...
            for iepoch in range(num_epochs):
                ...
                for step in range (int(train_size / BATCH_SIZE)):
                    ...
                    if step % RECORDING_STEP == 0:
                        ...
                        print('Epoch # ', iepoch+1, ' of ', num_epochs)
        ...
        if (TEST_RUN):
            ...
            for i in range(1, TEST_SIZE+1):
                ...
                print('Progress: ', i, str(TEST_SIZE))
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    if (CROSSVAL_RUN):
        ...
        train_score = [x[1] for x in results]
        plt.plot(np.linspace(0.1,0.9,5) ,train_score ,'r--',np.linspace(0.1,0.9,5),test_score, 'b--')
        plt.title('Train Score & Validation Score, cross validation threshold')
        plt.show()
